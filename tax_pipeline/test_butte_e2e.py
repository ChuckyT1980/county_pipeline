#!/usr/bin/env python3
"""Small Butte end-to-end sample: 3 books, 10 pages per book."""
...
BOOKS = [1, 2, 6]  # Using books 1, 2, 6 where we found parcels in the smoke test
                    ...
                    if kw.lower() in ln.lower():
                        for j in range(i+1, min(i+5, len(lines))):
                            if lines[j].lower() != kw.lower():
                                return lines[j]
                return None
            it = next((i for i, l in enumerate(lines) if "totals" in l.lower() or "total balance" in l.lower()), None)
            balance = None
            if it:
...
disc_csv = f"butte_sample_discovery_{ts}.csv"
