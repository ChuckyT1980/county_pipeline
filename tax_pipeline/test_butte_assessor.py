"""
Quick Butte assessor smoke test.
Verifies MPTS discovery API and tax detail pages work for Butte.
Run: python test_butte_assessor.py
"""
import requests
import time
import re
...
try:
    ...
    print(f"  search_url warm-up warning: {e}")
session.headers["Referer"] = search_url
time.sleep(1)

print(f"[{COUNTY.upper()}] Probing discovery API...")

# Try prefixes that should return parcels for Butte
probes = [f"{str(i).zfill(3)}010" for i in range(1, 20)] + [f"{str(i).zfill(3)}020" for i in range(1, 20)]
found = 0
sample_apn = None
...
if not sample_apn:
    print(f"\nNo parcels found in probe. {COUNTY.upper()} MPTS discovery may be unavailable or require different prefixes.")
    raise SystemExit(1)
