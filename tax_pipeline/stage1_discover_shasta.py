"""
Stage 1 (Shasta): Parcel discovery via feeparcel prefix enumeration.
Shasta's MBC lacks 'situs' search, so we enumerate 6-digit APN
prefixes (BBBPPP) to discover parcels across the county.

Incremental checkpointing: saves after every book in Phase 2.
Resume: run again with same output path — skips completed books.

Usage: python stage1_discover_shasta.py [output_csv]
"""
...
BASE = "https://common2.mptsweb.com/MBC/api/search/shasta/0000-CURR/feeparcel/"
S = requests.Session()
S.headers.update({
...
def get_parcels(prefix6):
    try:
        r = S.get(BASE + prefix6, timeout=10)
...
def norm(row):
    ...
    return {
        ...
        "fee_parcel": fp,
        "detail_url": f"https://common2.mptsweb.com/MBC/shasta/tax/main/{ac}/2025/0000",
        "county": "shasta", "discovered_at": datetime.utcnow().isoformat(),
    }

def discover(out_csv):
    ckpt_json = out_csv.replace(".csv", "_checkpoint.json")
    ckpt_csv  = out_csv.replace(".csv", "_partial.csv")

    print(f"\n[SHASTA DISCOVERY] -> {out_csv}", flush=True)
    warmup()

    # ── Phase 1: find valid books ──
    # Check if Phase 1 was already completed from a prior run
    valid_books = []
    ...
    if not valid_books:
        ...
        # Load completed books from checkpoint
        with open(ckpt_json) as f:
            meta = json.load(f)
        completed = set(meta.get("completed_books", []))
        # If partial CSV exists, also load its ASMTs to avoid re-query
        loaded_asmt = set()
        loaded_rows = []
    ...
    for book in remaining_books:
        ...
        # Save partial CSV + checkpoint after each book
    ...
    df = pd.DataFrame(loaded_rows)
    df.to_csv(out_csv, index=False)
    print(f"\nDONE: {len(df)} unique Shasta parcels -> {out_csv}", flush=True)
...
if __name__ == "__main__":
    ...
    discover(sys.argv[1] if len(sys.argv) > 1 else f"shasta_discovery_{ts}.csv")
