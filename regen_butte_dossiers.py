...
def build_parcel_outcome(apn: str, auction_row: dict, verified_row: dict):
    """Pure function, no file I/O - unit-testable directly with synthetic
    row dicts (Auction-Identity corrective implementation). Returns
    (parcel_data, None) for an eligible record, or (None, exclusion_outcome)
    for a redeemed one - exactly one of the two is non-None. Preserves the
    exact exclusion behavior main() already had; this only makes it
    testable in isolation and carries the literal redemption/power-to-sell
    fields through for eligible records instead of discarding them after
    the exclusion check."""
    a, v = auction_row, verified_row
    ...
    if is_redeemed(v):
        return None, {
            "apn": v.get("apn") or apn,
            "reconciliation_or_exclusion_reason": f"redemption_status={v.get('redemption_status')!r}",
            "redemption_status": v.get("redemption_status"),
            "redemption_date": v.get("redemption_date"),
        }
    ...
    source_ref = " + ".join(source_bits) or None
    ...
    parcel_data = {
        ...
        # Auction-Identity corrective implementation: these are eligibility/
        # tax-status evidence, not proof of an active auction listing - the
        # renderer's own gating and template wording make that distinction,
        # not this script.
        "redemption_status": v.get("redemption_status") or None,
        "redemption_date": v.get("redemption_date") or None,
        "power_to_sell_date": v.get("power_to_sell_date") or None,
        "status_source_artifact_ref": source_ref,
        "source_retrieval_timestamp": None,
        "freshness_status": "retrieval_time_unknown",
    }
    return parcel_data, None
...
def main():
    ...
    for apn in all_apns:
        ...
        parcel_data, exclusion_outcome = build_parcel_outcome(apn, a, v)
        if exclusion_outcome is not None:
            skipped_redeemed.append(exclusion_outcome)
            continue
    ...
    if skipped_redeemed:
        excluded_apns = [o["apn"] for o in skipped_redeemed]
        print(f"Skipped {len(skipped_redeemed)} redeemed parcel(s), excluded from output: {excluded_apns}")
