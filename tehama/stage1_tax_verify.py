...
# --- Config ---
COUNTY_SLUG = "tehama"
BASE_URL = "https://common1.mptsweb.com"
TAX_YEAR = "2024"
...
def fetch_tax_data(session, asmt_raw: str):
        ...
        if asr_resp.status_code == 200:
            ...
            for tr in soup.find_all("tr"):
                ...
                if len(cells) >= 2:
                    ...
                    elif label in ("Current Document Number", "Document Number") and not out["v_document_number"]:
                        out["v_document_number"] = cells[1].get_text(strip=True)
    ...
    try:
        ...
        if "ERROR" in resp.text[:500]:
            out["error"] = "MPTS Session Error / Not Found"
            return out
...
def run_stage1(input_csv, max_records=None):
    ...
    print("Initializing MPTS Session...")
    try:
        session.get(f"{BASE_URL}/MBC/{COUNTY_SLUG}/tax", timeout=15)
    except:
        pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_csv", nargs="?", default=os.path.join(os.path.dirname(__file__), "tehama_15_percent_sample.csv"))
