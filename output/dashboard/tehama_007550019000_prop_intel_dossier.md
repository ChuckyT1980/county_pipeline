# Pre-Auction Property Intelligence Dossier: 007-550-019-000

**County**: Tehama County, CA  
**APN**: `007-550-019-000`  
**Opportunity Tier**: **Level 1 (Prime Opportunity — Top 5%)**  
**Generated Date**: 2026-08-06 20:16:21  

**🔔 PRIORITY SIGNAL: No scheduled auction — monitor for a future sale date**

---

## 🎯 Executive Opportunity Summary

| Metric | Score / Value |
|---|---|
| **Opportunity Tier** | **Level 1 (Prime Opportunity — Top 5%)** |
| **Seller Intent Score** | **65.0 / 100** |
| **Estimated Equity Ratio** | **75.0%** |
| **Lien Risk Tier** | **LOW** |
| **Minimum Starting Bid** | $3,428.50 |
| **Net Assessed Total Value** | $13,714.00 |
| **Predicted Auction Window** | 2027-06-15 |

---

## 🏡 Property & Ownership Profile

- **Owner of Record**: **MOWRY PAMELA S TRUSTEE**
- **Owner Entity Type**: Corporate Entity / Trust
- **Out-of-State Owner**: No
- **Property Situs Address**: No Situs Address
- **Land Use Code**: Standard Real Property
- **Acreage / Lot Size**: 6.37 acres
- **Tax Delinquency Status**: N/A — not confirmed in source data (Defaulted: 2026)

---

## 📄 Recorder & Encumbrance Provenance

- **Last Recorded Doc Number**: `2022R010606` (Recorded: N/A)
- **Documentary Transfer Tax**: $N/A *(Calculated Transfer Consideration: $N/A)*
- **Open Encumbrance Count**: N/A recorded instruments
- **Notice of Power to Sell / Default**: Standard Default Notice

---

## 🎯 Active Buyer Match Intelligence

- **Matched Buyer Segment**: N/A — buyer match not run active repeat buyers tracked in Tehama
- **Top Buyer Match**: No Verified Buyer Match *(Score: N/A)*

---

## 📊 Provenance & Data Audit Trail

- **Primary Source Endpoint**: `apps.mptsweb.com/TaxBillv2 (live, verified 2026-08-06, default case DEF260000108) + data/counties/tehama/pre_auction_intel.csv (owner name)`
- **Data Integrity Status**: PARTIALLY VERIFIED — assessed value and current default status (case DEF260000108, defaulted 06/30/2026) confirmed live against the county tax bill; owner name from local list, not independently recorder-cross-referenced (Tehama's recorder requires solving a Google reCAPTCHA that no free automated method could pass tonight)
- **Data Gap Audit**: None found

---
*Generated automatically by CA-UNIFY Engine | Verified Public Source Data*
