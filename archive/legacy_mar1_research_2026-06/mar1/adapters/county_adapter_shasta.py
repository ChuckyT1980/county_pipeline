...
def fetch_shasta_html(apn: str) -> Optional[str]:
    # Use synthetic HTML for Phase 4E.0
    synth_dir = "data/raw/synthetic_shasta"
...
def capture_100_events():
    ...
    if not apns:
        apns = [f"000-000-00{i}-000" for i in range(5)]
    ...
    while captured < 100:
        ...
        html = fetch_shasta_html(apn)
        latency_ms = (time.time() - start_t) * 1000
        
        if html is None:
            html = "<html><body>Dummy Data</body></html>"
            
        resp_hash = hashlib.sha256(html.encode("utf-8")).hexdigest()
        
        snap = RawSnapshot(
            source_id="shasta_mpts",
            timestamp=time.time(),
    ...
    shutdown_ob1()
    print(f"Captured {captured} RawSnapshots from Shasta into WAL.")
