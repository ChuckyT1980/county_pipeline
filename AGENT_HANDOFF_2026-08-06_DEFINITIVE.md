...
**Recommended next batch of 4** (mpts-confirmed assessor, not yet attempted): a reasonable starting set would be humboldt (also tyler recorder, and there's an existing but empty `humboldt/` folder + a debunked "$635,951 verified" claim in older superseded handoff docs worth either substantiating for real or retiring), plus 3 more chosen by actually checking which have a real, current, findable auction list — do that check BEFORE spending build time, it's the fast/cheap step. Do not assume all 58 are worth doing; several almost certainly aren't (tiny population, no active online auction, or a recorder wall like Tehama's with no workaround). This triage step (does a real auction/default list even exist) is the correct first move for "4 at a time," not building scrapers blind.
...
8. The Humboldt "$635,951 verified" claim in older handoff docs does not hold up — the `humboldt/` working folder is empty. Don't trust it, and correct/retract it anywhere it's cited.
