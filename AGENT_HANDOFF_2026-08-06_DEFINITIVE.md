...
## Shasta — 6th county, real inventory as of 2026-08-07

23 real excess-proceeds leads in `shasta/shasta_excess_enrich.py`, sourced from Shasta's official "Notice of Right to Claim Excess Proceeds" (R&T 4676) at `shastacounty.gov/media/81921` — **this URL is behind a Cloudflare JS challenge**: `curl` gets a "Just a moment..." interstitial page and fails; a real/stealth browser passes it automatically and the page turns out to trigger a direct file download rather than rendering — handle with Playwright's `page.expect_download()` context manager (see the working pattern in the script, or in Tulare's below), not `page.goto()` alone. Real named parties-of-interest per parcel (the actual claimants — this is the hard part of a heir-finder business, already solved for these 23), but **no dollar amounts published** — Shasta only discloses the amount to an approved claimant after they file. Real exact claim deadline: 2027-03-22 (county-stated, not estimated).

**Found and fixed a real bug while building this**: `report_builder.py`'s `{{excess_amount}}` used to unconditionally format the amount as `$X.XX`, so a genuinely-undisclosed amount (Python `None`) rendered as `$0.00` — which reads as "confirmed zero," the opposite of the truth. Fixed to render `UNDISCLOSED BY COUNTY` when `excess_proceeds` is explicitly `None` vs. a real number. Applies to any future county with this same disclosure pattern.

## Tulare — 7th county, real inventory as of 2026-08-07

9 real excess-proceeds dossiers in `tulare/tulare_excess_enrich.py`, **$90,703.37 total** — the best-quality source found tonight: Tulare's official "March 3, 2026 Tax Auction Excess Proceeds Report" (`tc-web.widen.net/s/sbwzrsn592/2026-excess-proceeds-amounts-available`, linked from `tularecounty.ca.gov/auditor/2026-excess-proceeds`) has BOTH real assessee names AND real dollar amounts in the same document — Humboldt only had amounts, Shasta only had names. No CAPTCHA, no Cloudflare wall on this one; same `expect_download()` pattern as Shasta since the link triggers a direct file download. Tulare's `tc-web.widen.net` link pattern (a Widen DAM CDN, not the county's own domain) is likely reusable for other Tulare-published documents — check there first for anything else Tulare-related.

**General pattern now proven 3x tonight (Fresno, Humboldt, Shasta, Tulare — 4x)**: search `"<county> tax auction excess proceeds 2026"`, find the county's own Treasurer-Tax Collector/Auditor-Controller page, look for a results/excess-proceeds PDF or "amounts available" link. This is consistently faster AND higher-quality than building a scraper — try it FIRST for any new county before writing any extraction code. If the link triggers a download instead of rendering a page, use Playwright's `expect_download()` pattern (see either script above) — don't assume `page.goto()` alone will work, and don't assume a Cloudflare "Just a moment" curl response means the document is unreachable — try a real/stealth browser before concluding a wall is real.
